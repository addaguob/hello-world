...
void lock_pairs(void)
{
    ...
    */
    ...
    for (int i = 0; i < pair_count; i++) // loop from strongest pairs[i].winner
    {
        ...
        {
            // traverse back to each locked pair and back to current pair
            ...
            for (int j = 0; j < i; j++)               //                    T      T       T
            {
                if (locked[pairs[j].winner][pairs[j].loser])
                {
                    if (j == 0)
                    {
                        // if current winner was beaten in the past, begin cycle check
                        if (current_loser == locked_past_winner && locked[locked_past_winner][locked_past_loser])
                ...
                if (no_cycle_found && i <= graph_limit)
                {
                    locked[pairs[i].winner][pairs[i].loser] = true;
                }
            }
